...
function validateScrapeRange(fromDateStr, toDateStr) {
  ...
  let end;
  try {
    start = puppeteerService.parseDate(fromRaw);
    end = puppeteerService.parseDate(toRaw);
  } catch (e) {
    return {
      ok: false,
      status: 400,
...
module.exports = {
  ...
  async getRangeScrapeCoverage(fromDateStr, toDateStr) {
    const start = puppeteerService.parseDate(fromDateStr);
    const end = puppeteerService.parseDate(toDateStr);
    const step = start <= end ? 1 : -1;
    const d = new Date(start);
    const expectedDates = [];

    while (step > 0 ? d <= end : d >= end) {
      expectedDates.push(puppeteerService.formatDate(d));
