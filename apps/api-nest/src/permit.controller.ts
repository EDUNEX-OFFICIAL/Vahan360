...
export class PermitController {
  ...
  @Get('expiring')
  @ApiOperation({
    summary: 'Latest raw permit captures (ingest `RawPermit`)',
    description:
      'Returns recent `ingest.raw_permits` rows (newest `capturedAt` first, up to 100). `days` is reserved for future expiry filtering; echoed in the response. Clamped [1, 365]; default 30.',
  })
  @ApiQuery({
    name: 'days',
    required: false,
  ...
  getExpiring(
    ...
    return this.permitService.getExpiring(clamped, tenantId, from, to);
  }
}
