...
export type DistrictsSummaryRow = {
  id: string;
  district: string;
  snapshot: unknown;
  updatedAt: string;
...
@Injectable()
export class DistrictsService implements OnModuleDestroy {
  ...
  > {
    ...
    if (!prisma) {
      return {
        ...
        asOf,
        reason:
          'INGEST_DATABASE_URL or DATABASE_URL is not set; cannot load district summaries.',
      };
    }
    ...
    try {
      ...
      const dsub =
        districtSubstring != null && String(districtSubstring).trim() !== ''
          ? String(districtSubstring).trim()
          : undefined;

      const whereClause: Prisma.ProcessedDistrictSummaryWhereInput = {
        ...(dre ? { updatedAt: dre } : {}),
        ...(dsub
          ? {
              district: { contains: dsub, mode: 'insensitive' },
            }
          : {}),
      };

      const [rows, totalApprox] = await Promise.all([
        prisma.processedDistrictSummary.findMany({
          where: whereClause,
        ...
        prisma.processedDistrictSummary.count({ where: whereClause }),
      ]);
      return {
        status: 'ok',
        asOf,
