...
@Injectable()
export class RawVehicleService implements OnModuleDestroy {
  ...
  > {
    ...
    if (!prisma) {
      return {
        ...
        limit,
        reason:
          'INGEST_DATABASE_URL or DATABASE_URL is not set; cannot load raw vehicle records.',
      };
    }

    try {
      const tenantWhere = rawRowsLinkedToJobTenantWhere(tenantId) as Prisma.RawVehicleRecordWhereInput;
      const [rows, total] = await Promise.all([
        prisma.rawVehicleRecord.findMany({
          where: tenantWhere,
        ...
        prisma.rawVehicleRecord.count({ where: tenantWhere }),
      ]);
