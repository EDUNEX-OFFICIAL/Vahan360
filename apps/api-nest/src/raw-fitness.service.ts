...
@Injectable()
export class RawFitnessService implements OnModuleDestroy {
  ...
  async list(limit: number, tenantId: string): Promise<
    | {
        status: 'ok';
        asOf: string;
        limit: number;
  ...
  > {
    ...
    if (!prisma) {
      return {
        ...
        limit,
        reason:
          'INGEST_DATABASE_URL or DATABASE_URL is not set; cannot load raw fitness records.',
      };
    }

    try {
      const tenantWhere = rawRowsLinkedToJobTenantWhere(tenantId) as Prisma.RawFitnessRecordWhereInput;
      const [rows, total] = await Promise.all([
        prisma.rawFitnessRecord.findMany({
          where: tenantWhere,
        ...
        prisma.rawFitnessRecord.count({ where: tenantWhere }),
      ]);
