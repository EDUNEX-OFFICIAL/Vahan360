...
export default function RawVehiclePage() {
  ...
  const [rows, setRows] = useState<RawVehicleRow[] | null>(null);
  ...
  const load = useCallback(async (limitParam: string) => {
    ...
    const url = new URL(apiUrl('/api/v2/ingest/raw-vehicle'));
    ...
    try {
      ...
      if (data.status === 'ok' && Array.isArray(data.rows)) {
        ...
        setRows(data.rows as RawVehicleRow[]);
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold tracking-tight text-white">Raw Vehicle</h1>

          <p className="mt-1 text-sm text-slate-500">

            Nest ingest <code className="text-slate-400">RawVehicleRecord</code> via{' '}

            <code className="text-slate-400">GET /api/v2/ingest/raw-vehicle?limit=50</code> (Bearer{' '}
