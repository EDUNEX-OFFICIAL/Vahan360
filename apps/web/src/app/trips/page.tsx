...
> {
  ...
  try {
    ...
    if (data.status === 'ok' && Array.isArray(data.rows)) {
      return {
        variant: 'ok',
        rows: data.rows as TripSummaryApiRow[],
        meta: {
...
function TripsPageBody() {
  ...
  const qry = useQuery({
    queryKey: ['trips-summary', qsKey] as const,
    enabled: sessionOk,
    queryFn: async ({ queryKey }) => {
      const token = getSpybotToken();
      if (!token) throw new Error(NO_SPYBOT_JWT_MESSAGE);
      return fetchTrips({ qp: queryKey[1], token, router });
    },
  });
  ...
  function applyFilters() {
    router.replace(tripsHref(pathname, draftLimit, draftQ, draftFrom, draftTo), {
      scroll: false,
    });
  }
