...
export default function PermitsPage() {
  ...
  const [okRows, setOkRows] = useState<RawPermitSliceRow[] | null>(null);
  ...
  const fetchExpiring = async () => {
    ...
    const url = new URL(apiUrl('/api/v2/permits/expiring'));
    ...
    try {
      ...
      if (data.status === 'ok' && Array.isArray(data.rows)) {
        setOkRows(data.rows as RawPermitSliceRow[]);
        setOkMeta({
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-indigo-400">Permits</p>
          <h1 className="mt-2 text-2xl font-bold tracking-tight text-white">Permits expiring</h1>
          <p className="mt-1 text-sm text-slate-500">
            Nest ingest <code className="text-slate-400">RawPermit</code> via Express{' '}
            <code className="text-slate-400">/api/v2/permits/expiring?days=30</code>. Bearer{' '}
