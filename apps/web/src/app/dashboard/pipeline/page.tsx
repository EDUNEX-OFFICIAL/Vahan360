...
interface PipelineItem {
  ...
  status: PipelineTabStatus;
  nextFollowUp?: string;
  assignedExecutive?: string;
  updatedAt: string;
}

function normalizeStatus(value?: string): PipelineTabStatus {
  return normalizeSpringPipelineStatus(value);
}
...
const TABS: { id: PipelineTabStatus; label: string; icon: string; dotColor: string }[] = [
  {
    id: 'pending',
    label: 'Pending',
...
export default function PipelinePage() {
  const [activeTab, setActiveTab] = useState<PipelineTabStatus>('pending');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/vehicle/trip-summary?status=${activeTab}&limit=50`, token, {
        acceptJson: true,
      });
