...
function loginErrorMessage(data: Record<string, unknown>, status: number): string {
  ...
  if (status === 401) {
    return (
      ...
      'pnpm --filter @vahan360/api-express run sync:user — then try admin / admin123 (defaults).'
    );
  }
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch(apiUrl('/api/auth/generate-token'), {
        method: 'POST',
      ...
      setError(
        `Cannot reach the API at ${getApiBaseUrl()}. Check that the Express app is running and NEXT_PUBLIC_API_BASE_URL matches.`,
      );
    } finally {
      setSubmitting(false);
    }
