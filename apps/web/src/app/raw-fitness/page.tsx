...
export default function RawFitnessPage() {
  const router = useRouter();
  const [limit, setLimit] = useState(String(DEFAULT_LIMIT));
  const [loading, setLoading] = useState(true);
  const [rows, setRows] = useState<RawFitnessRow[] | null>(null);
  ...
  const load = useCallback(
    async (limitParam: string) => {
      ...
      const url = new URL(apiUrl('/api/v2/ingest/raw-fitness'));
      ...
      try {
        ...
        if (data.status === 'ok' && Array.isArray(data.rows)) {
          setRows(data.rows as RawFitnessRow[]);
          setMeta({
