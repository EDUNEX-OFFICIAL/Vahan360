...
function buildVehicleTripSummaryQuery(queryParams = {}) {
  ...
  const query = {};
  if (vehicleRegNo) {
    const esc = escapeRegex(String(vehicleRegNo));
    query.$or = [
      { vehicleRegNo: new RegExp(esc, 'i') },
      { ownerName: new RegExp(esc, 'i') },
    ];
  }
  if (ownerName) query.ownerName = new RegExp(escapeRegex(String(ownerName)), 'i');
  if (mobileNo) query.mobileNo = new RegExp(escapeRegex(String(mobileNo)), 'i');
  if (panNumber) query.panNumber = new RegExp(escapeRegex(String(panNumber)), 'i');
  if (make) query.make = new RegExp(escapeRegex(String(make)), 'i');
  if (model) query.model = new RegExp(escapeRegex(String(model)), 'i');
  ...
  if (currentDistrict) query.currentDistrict = new RegExp(escapeRegex(String(currentDistrict)), 'i');
  if (permanentDistrict) query.permanentDistrict = new RegExp(escapeRegex(String(permanentDistrict)), 'i');
  if (currentPincode) query.currentPincode = new RegExp(escapeRegex(String(currentPincode)), 'i');
  if (permanentPincode) query.permanentPincode = new RegExp(escapeRegex(String(permanentPincode)), 'i');
  if (assignedExecutive) query.assignedExecutive = new RegExp(escapeRegex(String(assignedExecutive)), 'i');
  if (leadSource) query.leadSource = new RegExp(escapeRegex(String(leadSource)), 'i');
  if (insuranceCompany) query.insuranceCompany = new RegExp(escapeRegex(String(insuranceCompany)), 'i');
